...
int main()
{
	RandomFileGenerator("data.txt",10000);					//Generate a random file

	vector<string> tempFiles = SplitFile("DATA.txt",1000);	//Split the Source File into number of Temp files
	
	MergeFiles(tempFiles,"BETA.txt");						//Merge the Temp Files into one Dest File
...
void RandomFileGenerator(string fname,int contentLength)
{
	contentLength = contentLength == 0 ? 1 : contentLength;//ensuring contentlength is not zero
	...
	int x = contentLength-1;
	
	fout << rand() % contentLength;
	while (x--)
	{
		fout << "\n";
		fout << rand() % contentLength;
